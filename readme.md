...
Прошивка донгла публикует по BLE два отдельных значения Battery Level — по одному для каждой половинки. macOS обычно показывает рядом с именем устройства только одно значение, поэтому для одновременного просмотра обоих уровней в репозитории есть терминальная программа. По умолчанию она запускает полноэкранный TUI:

```sh
make battery-tui
```

Для запуска нужен установленный [uv](https://docs.astral.sh/uv/). Зависимость `bleak` будет установлена `uv` в изолированное окружение. При первом запуске macOS запросит у Terminal доступ к Bluetooth.

По умолчанию программа ищет BLE-устройство с именем `Charybdis` и считает, что половинки были привязаны к донглу в порядке «левая, правая». Имя устройства и подписи можно изменить:

```sh
make battery-tui BATTERY_TUI_ARGS='--name Charybdis --labels Левая,Правая'
```

Для проверки подключения и чтения заряда без полноэкранного интерфейса используйте одноразовый текстовый режим:

```sh
make battery-read
```

Единую команду `chbat` можно установить в `~/.local/bin` (каталог должен присутствовать в `PATH`):
...
Без аргументов команда запускает TUI. Режим можно выбрать явно, а для быстрой проверки использовать короткий вывод:
...
Установщик находится в `scripts/install-chbat`; Make-цель только вызывает его. При необходимости каталог установки можно изменить:

```sh
CHBAT_BIN_DIR="$HOME/bin" ./scripts/install-chbat
```

На macOS утилита сначала проверяет уже подключённые системные BLE-устройства, поскольку подключённая клавиатура обычно больше не видна при обычном сканировании рекламы.

После перепрошивки донгла нужно удалить старое BLE-сопряжение `Charybdis` в настройках macOS и создать его заново: macOS кэширует список GATT-сервисов и иначе может продолжить показывать старый заряд донгла `100%`.

В TUI нажмите `r` для повторного поиска/подключения и `q` для выхода. Если обе половинки были привязаны в обратном порядке, передайте `--labels Правая,Левая`.
