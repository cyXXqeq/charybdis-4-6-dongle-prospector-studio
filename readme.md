# Charybdis 4x6 with Dongle and ZMK STUDIO
...
## Просмотр заряда обеих половинок по BLE

Прошивка донгла публикует по BLE два отдельных значения Battery Level — по одному для каждой половинки. macOS обычно показывает рядом с именем устройства только одно значение, поэтому для одновременного просмотра обоих уровней в репозитории есть команда `chbat`.

Для работы нужен установленный [uv](https://docs.astral.sh/uv/). Установите команду в `~/.local/bin`:

```sh
make install-battery-cli
```

Установщик создаёт символическую ссылку `~/.local/bin/chbat` на версию утилиты из репозитория. Если `~/.local/bin` ещё не входит в `PATH`, добавьте его в `~/.zshrc` и перезапустите терминал:

```sh
export PATH="$HOME/.local/bin:$PATH"
```

После установки доступны полноэкранный TUI и короткий одноразовый вывод:

```sh
chbat           # TUI по умолчанию
chbat --tui     # TUI явно
chbat --short   # заряд обеих половинок одной строкой
```

Устанавливать команду необязательно: те же режимы можно запустить прямо из репозитория:
...
Мощный исправный USB-C PD-адаптер, включая UGREEN CD333 на 300 Вт, не подаёт всю заявленную мощность принудительно и подходит для зарядки nice!nano. Требования к адаптеру и аккумулятору, настройка имени устройства и подписей половинок, установка в другой каталог, горячие клавиши TUI и решение проблем с BLE описаны в [подробном руководстве](docs/battery-monitor.md).
...
![12323232](https://github.com/devpew/charybdis-4-6-dongle-prospector-studio/blob/main/img/22.png)
...
Исключение — изменение `matrix transform` в `boards/shields/charybdis/charybdis.dtsi`. Преобразование матрицы также компилируется в прошивки половинок: они переводят нажатие в номер позиции до отправки события донглу. После изменения порядка `RC(...)` нужно прошить одним комплектом донгл, левую и правую половинки, иначе назначения клавиш будут перепутаны.
...
## Порядок прошивки клавиатуры

В конфигурации с донглом используются три контроллера: правая половина, левая половина и центральный донгл. Прошивать всегда все три не нужно — набор зависит от измененного файла:

- изменения только в keymap и комбо — только донгл;
- `charybdis_right.conf` или `charybdis_3610.dtsi` (PMW3610, CPI, частота отчетов) — только правая половина;
- `split_input_common.dtsi` — правая половина и донгл;
- `matrix transform`, первое включение ZMK STUDIO, несовместимый комплект прошивок или проблемы с BLE-сопряжением — все три контроллера из одной сборки.

### Обновление исправления «пьяного» курсора

Ограничение частоты отчетов PMW3610 находится только в `config/charybdis_right.conf`, поэтому для этого изменения достаточно правой половины. `settings_reset` использовать не нужно: он сотрет сохраненное BLE-сопряжение.

1. Соберите правую половину командой `make zmk-right` или скачайте артефакты GitHub Actions для нужного коммита.
2. Подключите правую половину по USB и дважды нажмите reset, чтобы открыть накопитель загрузчика.
3. Скопируйте на него `charybdis_right-nice_nano-zmk.uf2`. При локальной сборке файл находится в `artifacts/firmware/`.
4. Дождитесь закрытия накопителя, отключите USB и перезапустите правую половину выключателем.
5. Проверьте движение и прокрутку трекбола. Донгл и левую половину перепрошивать не нужно.

### Полная чистая перепрошивка и повторное сопряжение

Эта процедура нужна при первом запуске полного комплекта, после несовместимых прошивок или если донгл перестал видеть половины. Она удаляет прежние BLE-бонды.

1. Соберите полный комплект командой `make zmk-all` или скачайте один набор артефактов из одного запуска GitHub Actions.
2. Отключите донгл от компьютера и выключите обе половины.
3. Подключите правую половину, войдите в bootloader и прошейте `settings_reset-nice_nano-zmk.uf2`.
4. Снова войдите в bootloader, прошейте `charybdis_right-nice_nano-zmk.uf2`, отключите USB и оставьте правую половину выключенной.
5. Аналогично прошейте левую половину: сначала `settings_reset-nice_nano-zmk.uf2`, затем `charybdis_left-nice_nano-zmk.uf2`; отключите USB и оставьте ее выключенной.
6. Убедитесь, что обе половины выключены.
7. Подключите донгл и сначала прошейте подходящий reset-файл: `settings_reset-nice_nano-zmk.uf2` для Nice Nano либо `settings_reset-xiao_ble-zmk.uf2` для XIAO/Prospector.
8. Снова войдите в bootloader и прошейте соответствующий файл донгла: Nice Nano, XIAO BLE или Prospector из того же набора артефактов.
9. Положите донгл и обе половины рядом, примерно в 10–20 см друг от друга, и одновременно включите обе половины. Сопряжение должно завершиться через несколько секунд.
...
Для изменения чувствительности трекбола поменяйте значение `cpi` в `boards/shields/charybdis/charybdis_3610.dtsi`. Этот sensor node компилируется только в прошивку правой половины, поэтому после изменения CPI прошейте только `charybdis_right-nice_nano-zmk.uf2` по короткой инструкции выше.

Для защиты от эффекта «пьяного» курсора в `config/charybdis_right.conf` включено `CONFIG_PMW3610_ALT_REPORT_INTERVAL_MIN=12`. Драйвер продолжает считывать перемещение сенсора, но объединяет частые XY-события перед отправкой с правой половины на донгл по split BLE. Подробная диагностика описана в [docs/debugging.md](docs/debugging.md#пьяный-курсор-трекбола).
